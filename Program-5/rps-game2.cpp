#include <iostream>	 /* File: game.cpp */
#include <cstdlib>   // Info about random number generator rand() 
using namespace std;	

int ROCK = 0, PAPER = 1, SCISSORS = 2; // 0/1/2 to represent ROCK/PAPER/SCISSORS
...
int print_choice(char player, int choice) // 'C' for computer and 'U' for user
{
    ...
    {
        cerr << "Error: Invalid user --- " << player << endl; return -1;
    }
    cout << " picks ";
    ...
    {
        cerr << "Error: Invalid game choice --- " << choice << endl; return -1;
    }
